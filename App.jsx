...
/* ─── CLASS CODE ──
   Short, easy-to-share alphanumeric code that links a student to their teacher.
   Generated when a teacher signs up; students enter it on self-signup so they
   join the right class automatically (no orphans).
   Avoids visually-confusing characters (0, O, 1, I) so kids can read codes off
   a whiteboard without typos. */
...
// Look up a teacher by their class code (case-insensitive, ignores whitespace).
function findTeacherByClassCode(teachers, codeInput) {
  if (!codeInput) return null;
  const normalized = String(codeInput).trim().toUpperCase().replace(/\s+/g, "");
  return teachers.find(t => (t.classCode || "").toUpperCase() === normalized) || null;
}
...
function QuizGame({ studentId, studentName, quiz, mission, savedProgress, missionFullCount, missionAlreadyAnswered, onClose, onComplete, onSaveProgress, onQuestionAnswered, onAnswerAttempt, onShop }) {
  ...
  const totalReward = source?.points || 1;
  ...
  const nextQuestion = () => {
    if (isLast) {
      ...
      if (!rewarded) {
        ...
        onComplete({ pointsEarned: earned, scoreCorrect: score, scoreTotal: questions.length, wrongAnswers });
      }
    } else {
      SFX.click();
      setCurrentIdx(i => i + 1);
          ...
          setLives(l => {
            ...
            if (next <= 0 && !rewardedRef.current) {
              ...
              onComplete({
                ...
                scoreCorrect: questionsAnsweredRef.current,
      ...
      if (isCorrect) {
        setQuestionsAnswered(q => {
          ...
          if (newCount >= targetQuestions && !rewardedRef.current) {
            ...
            setRewarded(true);
            SFX.levelUp();
            onComplete({ pointsEarned: totalReward, won: true, questionsAnswered: newCount, scoreCorrect: newCount, scoreTotal: targetQuestions, wrongAnswers });
          }
          return newCount;
        });
      } else {
...
function CrushGame({ studentName, mission, savedProgress, missionFullCount, missionAlreadyAnswered, onClose, onComplete, onSaveProgress, onQuestionAnswered, onAnswerAttempt, onShop }) {
  ...
  const [wrongAnswers, setWrongAnswers] = useState([]); // {questionIdx, question, chosen, chosenText, correct, correctText}
      ...
      if (isCorrect) {
        ...
        setQuestionsCompleted(q => {
          ...
          if (newCount >= targetQuestions && !rewarded) {
            ...
            setGameOver(true);
            SFX.levelUp();
            onComplete({ pointsEarned: totalReward, won: true, scoreCorrect: targetQuestions, scoreTotal: targetQuestions, wrongAnswers });
          }
          return newCount;
        });
      }
...
/* ─── TUTORIAL CONTENT ───
   Per-game step lists used by TutorialPrompt + TutorialOverlay below.
   Keep each step very short — these are read by kids who may have limited
   reading speed / English fluency. */
const TUTORIAL_STEPS = {
  blockblast: [
    { emoji: "🧩", title: "Drag a block", text: "Pick a coloured block from the tray and drag it onto the grid." },
    { emoji: "📏", title: "Fill a line", text: "Complete a whole row or column to clear it and earn stars." },
    { emoji: "❓", title: "Answer the question", text: "After clearing a line, a question pops up. Get it right to keep your streak!" },
  ],
  runner: [
    { emoji: "🏃", title: "Keep running", text: "Your monkey runs by itself. Tap the screen or press SPACE to jump." },
    { emoji: "🌳", title: "Dodge things", text: "Watch for trees and rocks. Jump over them — don't bump in!" },
    { emoji: "❓", title: "Answer questions", text: "A question appears as you run. Pick the right answer to keep going strong!" },
  ],
  flappy: [
    { emoji: "❄️", title: "Flap to fly", text: "Tap or press SPACE to flap. Each tap pushes you up a little." },
    { emoji: "🏔️", title: "Mind the icicles", text: "Fly through the gap between the icicles." },
    { emoji: "❓", title: "Smart birds win", text: "Stop to answer questions — correct answers earn more stars." },
  ],
  crush: [
    { emoji: "🌸", title: "Swap two tiles", text: "Tap a tile, then tap a tile next to it to swap them." },
    { emoji: "✨", title: "Match 3 or more", text: "Line up three or more of the same tile to clear them." },
    { emoji: "❓", title: "Answer pop-ups", text: "Questions appear as you play. Right answers give you bonus rewards!" },
  ],
  quiz: [
    { emoji: "📖", title: "Read carefully", text: "Read the question. Take your time — there's no rush." },
    { emoji: "👆", title: "Pick an answer", text: "Tap the answer you think is correct." },
    { emoji: "⚡", title: "Speed bonus", text: "Fast correct answers earn extra stars — but only if they're right." },
  ],
  tetris: [
    { emoji: "⬇️", title: "Pieces fall", text: "Blocks drop from the top. Move them left/right with the arrow keys." },
    { emoji: "📏", title: "Fill a row", text: "Fill a whole row to clear it and earn stars." },
    { emoji: "❓", title: "Answer questions", text: "Questions pop up between rounds. Answer to earn more stars!" },
  ],
};
...
const GAME_TYPE_EMOJI = {
  blockblast: "🧩", runner: "🏃", flappy: "❄️", crush: "🌸", quiz: "📝", tetris: "🟦",
};

/* ─── TutorialPrompt ───
   Quick yes/skip modal shown the first time a student opens a given game type.
   "Skip" persists so they aren't asked again; "Yes" starts the bubble walkthrough. */
function TutorialPrompt({ gameType, onYes, onSkip }) {
  const label = GAME_TYPE_LABELS[gameType] || "this game";
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: "0 0 4px", color: C.text, fontSize: 24 }}>
          First time playing {label}?
        </h2>
        <p style={{ margin: "0 0 18px", color: C.textLight, fontSize: 15, lineHeight: 1.4 }}>
          Would you like a quick tutorial? It takes about 20 seconds.
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <button onClick={onSkip} style={{
            padding: "11px 22px", borderRadius: 14,
            background: "transparent", border: `2px solid ${C.fur2}40`,
...
/* ─── TutorialOverlay ───
   Renders 3 step-by-step speech bubbles on top of a running game.
   Bubbles are centered cards (not anchored to specific DOM nodes) so they
   work across every game without coupling to game internals. */
function TutorialOverlay({ gameType, onDone }) {
  const steps = TUTORIAL_STEPS[gameType] || TUTORIAL_STEPS.quiz;
  ...
  return (
    ...
    >
      <div style={{
        background: C.card, borderRadius: 22, padding: "22px 26px",
        ...
        <div style={{
          position: "absolute", bottom: -10, left: "50%", transform: "translateX(-50%) rotate(45deg)",
          width: 18, height: 18, background: C.card, borderRight: `2px solid ${C.gold}40`, borderBottom: `2px solid ${C.gold}40`,
        }} />
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
          <div style={{ fontSize: 38 }}>{step.emoji}</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, color: C.textLight, fontWeight: 700, letterSpacing: 1 }}>
              STEP {stepIdx + 1} OF {steps.length}
            </div>
            <div style={{ fontSize: 22, color: C.text, fontWeight: 700, lineHeight: 1.1 }}>
              {step.title}
            </div>
          </div>
        </div>
        <p style={{ margin: "4px 0 14px", color: C.text, fontSize: 16, lineHeight: 1.4 }}>
          {step.text}
        </p>
        <div style={{ display: "flex", gap: 8, alignItems: "center", justifyContent: "space-between" }}>
          <button onClick={onDone} style={{
            padding: "7px 14px", borderRadius: 10, background: "transparent",
            border: "none", color: C.textLight,
            fontFamily: "'Patrick Hand', cursive", fontSize: 14, cursor: "pointer",
          }}>
            Skip tutorial
          </button>
          <div style={{ display: "flex", gap: 4 }}>
            {steps.map((_, i) => (
              <span key={i} style={{
                ...
                background: i === stepIdx ? C.gold : `${C.fur2}40`,
              }} />
            ))}
          </div>
          <button onClick={() => { if (isLast) onDone(); else setStepIdx(stepIdx + 1); }}
            style={{
              padding: "10px 22px", borderRadius: 12,
              background: C.green, border: "none", color: "white",
              fontFamily: "'Patrick Hand', cursive", fontSize: 16, fontWeight: 700,
              cursor: "pointer", boxShadow: `0 3px 10px ${C.green}55`,
            }}>
            {isLast ? "Got it!" : "Next →"}
          </button>
        </div>
...
/* ─── TeacherRoster ───
   Minimalist, spreadsheet-style alphabetical view of every student in the
   class. This is the new default teacher landing view — replaces the
   "hot spring scene" as the first thing teachers see after login.
   Designed for teachers with limited tech skills: one screen, clear actions,
   alphabetical sort, no decorative monkeys cluttering the data.

   Props:
     students        — myClassStudents
     missions        — { studentId: [missions] }
     onAssignPack    — () => open StudyPacksBrowser
     onOpenYearbook  — () => open TeacherOverview
     onOpenManage    — () => open Manage panel (existing)
     onSelectStudent — (studentId) => focus a student (existing per-student panel)
     onAddStudent    — () => open add-student panel
     onAddMission    — (studentId) => open mission upload
     onAdjustPoints  — (studentId, delta)
*/
function TeacherRoster({
  students, missions, onAssignPack, onOpenYearbook, onOpenManage,
  onSelectStudent, onAddStudent, onAddMission, onAdjustPoints, onSwitchToHotSpring,
}) {
  const [query, setQuery] = useState("");
  const [sortKey, setSortKey] = useState("name"); // "name" | "points" | "missions" | "last"

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    const filtered = q
      ? students.filter(s => (s.name || "").toLowerCase().includes(q) || (s.username || "").toLowerCase().includes(q))
      : students;
    const enriched = filtered.map(s => {
      const mList = missions[s.id] || [];
      const total = mList.length;
      const done = mList.filter(m => s.completions?.[`mission:${m.id}`]?.completed).length;
      const lastAttempt = Math.max(0, ...mList.map(m => s.completions?.[`mission:${m.id}`]?.lastAttempt || 0));
      return { s, total, done, lastAttempt };
    });
    enriched.sort((a, b) => {
      if (sortKey === "points") return (b.s.points || 0) - (a.s.points || 0);
      if (sortKey === "missions") return (b.done - b.total) - (a.done - a.total) || (b.total - a.total);
      if (sortKey === "last") return b.lastAttempt - a.lastAttempt;
      return (a.s.name || "").localeCompare(b.s.name || "");
    });
    return enriched;
  }, [students, missions, query, sortKey]);
  ...
  const SortBtn = ({ k, label }) => (
    <button onClick={() => setSortKey(k)} style={{
      padding: "4px 10px", borderRadius: 999,
      background: sortKey === k ? C.text : "transparent",
      color: sortKey === k ? C.card : C.textLight,
      border: `1.5px solid ${sortKey === k ? C.text : C.fur2 + "40"}`,
      fontFamily: "'Patrick Hand', cursive", fontSize: 13, fontWeight: 700,
      cursor: "pointer", transition: "all 0.15s",
    }}>{label}</button>
  );
  ...
  return (
    <div style={{
      margin: "8px auto 0", width: "96%", maxWidth: 1300,
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          <h2 style={{ margin: "0 0 2px", color: C.text, fontSize: 24 }}>
            Class Roster
          </h2>
          <p style={{ margin: 0, color: C.textLight, fontSize: 13 }}>
            {students.length} student{students.length === 1 ? "" : "s"} · click a name to give points or open their profile
          </p>
        </div>
        <input
          ...
          style={{
            ...
            border: `1.5px solid ${C.fur2}30`, background: C.snow1,
            fontFamily: "'Patrick Hand', cursive", fontSize: 15,
            color: C.text, minWidth: 200,
          }}
        />
        <button onClick={onAddStudent} style={{
          ...
          cursor: "pointer", boxShadow: `0 3px 10px ${C.green}40`,
        }}>+ Add Student</button>
        <button onClick={onAssignPack} style={{
          padding: "10px 18px", borderRadius: 12, border: `2px solid ${C.gold}50`,
          background: `${C.gold}15`, color: C.text,
          fontFamily: "'Patrick Hand', cursive", fontSize: 15, fontWeight: 700,
          cursor: "pointer",
        }}>📚 Assign a Pack</button>
      </div>
      ...
      {/* Spreadsheet */}
      <div style={{
        border: `1px solid ${C.fur2}25`, borderRadius: 14, overflow: "hidden",
      }}>
        {/* Header row */}
        <div style={{
          display: "grid",
          gridTemplateColumns: "minmax(180px, 2fr) 90px 110px 130px 220px",
          gap: 10, padding: "10px 14px",
          background: `${C.snow1}80`, borderBottom: `1.5px solid ${C.fur2}25`,
          fontSize: 12, fontWeight: 700, color: C.textLight, letterSpacing: 0.5,
        }}>
          <div>NAME</div>
          <div style={{ textAlign: "right" }}>★ STARS</div>
          <div style={{ textAlign: "right" }}>MISSIONS</div>
          <div style={{ textAlign: "right" }}>LAST ACTIVE</div>
          <div style={{ textAlign: "right" }}>ACTIONS</div>
        </div>

        {rows.length === 0 && (
          <div style={{ padding: 40, textAlign: "center", color: C.textLight, fontSize: 15 }}>
            {query ? "No students match your search." : "No students yet — click + Add Student to get started."}
          </div>
        )}

        {rows.map(({ s, total, done, lastAttempt }, idx) => {
          ...
          return (
            <div key={s.id} style={{
              ...
              alignItems: "center",
              background: idx % 2 === 0 ? C.card : `${C.snow1}30`,
              borderBottom: idx === rows.length - 1 ? "none" : `1px solid ${C.fur2}15`,
              transition: "background 0.15s",
            }}
              onMouseEnter={e => e.currentTarget.style.background = `${C.water1}10`}
              onMouseLeave={e => e.currentTarget.style.background = idx % 2 === 0 ? C.card : `${C.snow1}30`}>
              <div onClick={() => onSelectStudent(s.id)} style={{ cursor: "pointer" }}>
                <div style={{ fontSize: 17, color: C.text, fontWeight: 700, lineHeight: 1.15 }}>
                  {s.name} {allDone && <span style={{ color: C.gold, fontSize: 14 }}>🏆</span>}
                </div>
                <div style={{ fontSize: 12, color: C.textLight }}>@{s.username}</div>
              </div>
              <div style={{ textAlign: "right", fontSize: 17, color: C.gold, fontWeight: 700 }}>
                ★ {s.points || 0}
              </div>
              <div style={{ textAlign: "right", fontSize: 14, color: C.text }}>
                {done}/{total}
                {total > 0 && (
                  <div style={{
                    height: 4, background: `${C.fur2}20`, borderRadius: 2,
                    marginTop: 3, overflow: "hidden",
                  }}>
                    <div style={{
                      width: `${(done / total) * 100}%`, height: "100%",
                      background: allDone ? C.gold : C.green,
                      transition: "width 0.3s",
                    }} />
                  </div>
                )}
              </div>
              <div style={{ textAlign: "right", fontSize: 13, color: C.textLight }}>
                {relTime(lastAttempt)}
              </div>
              <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                <button onClick={() => onAdjustPoints(s.id, +1)} title="Give 1 star" style={{
                  padding: "6px 10px", borderRadius: 9, border: "none",
                  background: `${C.green}25`, color: C.green,
                  fontFamily: "'Patrick Hand', cursive", fontSize: 13, fontWeight: 700, cursor: "pointer",
                }}>+1 ★</button>
                <button onClick={() => onAdjustPoints(s.id, -1)} title="Take 1 star" style={{
                  padding: "6px 10px", borderRadius: 9, border: "none",
                  background: `${C.accent}20`, color: C.accent,
                  fontFamily: "'Patrick Hand', cursive", fontSize: 13, fontWeight: 700, cursor: "pointer",
                }}>−1 ★</button>
                <button onClick={() => onAddMission(s.id)} title="Add a mission for this student" style={{
                  padding: "6px 12px", borderRadius: 9, border: "none",
                  background: `${C.water1}30`, color: C.text,
                  fontFamily: "'Patrick Hand', cursive", fontSize: 13, fontWeight: 700, cursor: "pointer",
                }}>🚀 Assign</button>
              </div>
            </div>
...
function SnowMonkeyTrackerInner() {
  ...
  const [signupClassCode, setSignupClassCode] = useState(""); // student-side: which teacher's class to join
  ...
  // ─── CLASS-SCOPED STUDENT FILTER ──
  // Each teacher owns one "class" — the students they created. A student belongs
  // to a teacher via `student.teacherId`. The teacher's UI only shows their
  // own class students; admins (isAdmin: true) see everyone.
  //
  ...
  // When a teacher logs in (or data loads) and there are orphan students,
  // pop up the assignment modal automatically. Only fires once per session
  // per teacher login.
  ...
  useEffect(() => {
    ...
    if (orphanStudents.length > 0 && !orphanShownRef.current) {
  ...
  useEffect(() => {
    ...
    (async () => {
      // Pick a code that doesn't collide with any existing teacher
      let code;
      for (let i = 0; i < 10; i++) {
        const c = generateClassCode();
        if (!teachers.find(t => t.classCode === c)) { code = c; break; }
      }
      if (!code) code = generateClassCode();
      // Optimistic local update first
      setTeachers(prev => prev.map(t => t.id === user.id ? { ...t, classCode: code } : t));
      setUser(u => u ? { ...u, classCode: code } : u);
      // Persist to Firestore (best-effort — if it fails, local code is still usable)
      try {
        await updateTeacher(user.id, { classCode: code });
      } catch (e) {
        console.warn("Class code backfill: persist failed (will retry on next login):", e?.message);
      }
    })();
  }, [user, teachers]);
  ...
  // "roster"   — minimalist spreadsheet view of all students (the new default;
  //              easier for teachers with limited tech skills).
  // "hotspring"— the original animated hot-spring scene with monkeys swimming.
  // Teachers can flip between them via the toolbar.
  const [teacherView, setTeacherView] = useState("roster");
  // ── Tutorial state ──
  // `tutorialPromptDecided` tracks which (missionId+gameType) combos the student
  // has already decided about THIS SESSION (prevents re-prompting). Persistence
  // for "I've seen this tutorial forever" lives in user.completions["tutorial:XYZ"].
  const [tutorialPromptDecided, setTutorialPromptDecided] = useState({});
  // When non-null, render the TutorialOverlay (bubble walkthrough) over the
  // currently-running game for this game type.
  const [activeTutorialGameType, setActiveTutorialGameType] = useState(null);
  ...
  const [missionResultsTarget, setMissionResultsTarget] = useState(null);
  ...
  const [leaderboardTab, setLeaderboardTab] = useState("class"); // "class" | "global"
  ...
  // ── Stable callbacks (perf) ──
  // The mission dispatcher used to define these inline every render, which
  // caused the heavy game components to remount/re-render on any parent state
  // change (notifications, leaderboard ticks, sound toggles, etc.). Hoisting
  // them to useCallback keeps the prop identity stable so React.memo (and the
  // games' own internal effect deps) can short-circuit unnecessary work.
  ...
  const openFoodShop = useCallback(() => setShowFoodShop(true), []);

  // Persists the "I've seen this game's tutorial" flag so we don't ask twice.
  // Stored under completions["tutorial:<gameType>"] = { completed, seenAt }.
  const persistTutorialSeen = useCallback(async (gameType) => {
    if (!user || !gameType) return;
    const tutorialKey = `tutorial:${gameType}`;
    const completions = { ...(user.completions || {}) };
    if (completions[tutorialKey]?.completed) return; // already saved
    completions[tutorialKey] = { completed: true, seenAt: Date.now() };
    const newS = students.map(s => s.id === user.id ? { ...s, completions } : s);
    setStudents(newS);
    setUser(u => u && u.id === user.id ? { ...u, completions } : u);
    try { await updateStudent(user.id, { completions }); }
    catch (e) { console.warn("Tutorial-seen save failed:", e?.message); }
  }, [user, students]);
  ...
  const handleMissionComplete = async (result) => {
    ...
    const scoreCorrect = (typeof r.scoreCorrect === "number") ? r.scoreCorrect : null;
    const scoreTotal = (typeof r.scoreTotal === "number") ? r.scoreTotal : null;
    const activeMission = (missions[user.id] || []).find(m => m.id === activeMissionId);
    // Lock reward to first completion (replays don't grant more stars)
    const alreadyCompleted = !!(user?.completions?.[`mission:${activeMissionId}`]?.completed);
    const grantedStars = (won && !alreadyCompleted) ? pointsEarned : 0;
    const newS = students.map(s => {
      if (s.id !== user.id) return s;
      const completions = { ...(s.completions || {}) };
      if (activeMission) {
        ...
        completions[perGameKey] = {
          ...
          lastAttempt: Date.now(),
          totalEarned: (prevPerGame.totalEarned || 0) + grantedStars,
        ...
        // Aggregate "this mission was completed at least once" for reminders + teacher results
        const prevAgg = completions[aggregateKey] || { attempts: 0, totalEarned: 0, completed: false };
        const latestAttempt = won ? {
          gameType,
          score: scoreCorrect,
          scoreTotal: scoreTotal,
          wrongAnswers,
          completedAt: Date.now(),
        } : (prevAgg.latestAttempt || null);
        // Best-score tracking: keep the highest scoreCorrect across all attempts
        const bestScore = (won && scoreCorrect !== null)
          ? Math.max(prevAgg.bestScore || 0, scoreCorrect)
          : (prevAgg.bestScore || 0);
        completions[aggregateKey] = {
          ...
          lastAttempt: Date.now(),
          totalEarned: (prevAgg.totalEarned || 0) + grantedStars,
          completed: prevAgg.completed || won,
          latestAttempt,
          bestScore,
          scoreTotal: scoreTotal || prevAgg.scoreTotal || null,
        };
      }
      return { ...s, points: s.points + grantedStars, completions };
    });
    persist(null, newS);
  ...
  const handleManualSignup = async () => {
    ...
    try {
      if (signupRole === "student") {
        // If a class code was entered, look up the teacher it belongs to.
        // Invalid codes block signup (better UX than letting them in as an orphan).
        let resolvedTeacherId = null;
        const codeInput = (signupClassCode || "").trim();
        if (codeInput) {
          const teacher = findTeacherByClassCode(teachers, codeInput);
          if (!teacher) {
            setSignupError("Class code not found. Double-check with your teacher.");
            setSignupBusy(false);
            return;
          }
        ...
        const newStudent = await addStudentToDB({
          ...
          ownedPets: [],
          pet: null,
          teacherId: resolvedTeacherId, // null if no code; a teacher can claim them later
        });
        setStudents([...students, newStudent]);
        setUser(newStudent);
        setScreen("student");
        setShowSignupModal(false);
        setSignupName(""); setSignupUser(""); setSignupPass(""); setSignupClassCode("");
        ...
        const newTeacher = await addTeacherToDB({
          ...
          classCode: (() => {
            for (let i = 0; i < 10; i++) {
              ...
              if (!teachers.find(t => t.classCode === code)) return code;
            }
            return generateClassCode(); // 0.0001% collision chance, accept it
          })(),
        });
        ...
        notify(`Welcome, ${newTeacher.name}! 🎉  Your class code is ${newTeacher.classCode}`);
      }
    } catch (e) {
  ...
  const addStudent = async () => {
    ...
    try {
      ...
      notify(`${newStudentName.trim()} joined your class!`);
    } catch (error) {
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <input value={signupPass} onChange={e => setSignupPass(e.target.value)}
                  ...
                  style={inputStyle} />
                {/* Class code: only shown for student signup.
                    Optional — if blank, the student remains an orphan and won't see any
                    teacher's missions until a teacher claims them via the orphan modal. */}
                {signupRole === "student" && (
                  <div>
                    <input
                      value={signupClassCode}
                      onChange={e => setSignupClassCode(e.target.value.toUpperCase())}
                      placeholder="Class code from your teacher (optional)"
                      maxLength={8}
                      style={{ ...inputStyle, letterSpacing: 2, fontWeight: 700, textAlign: "center" }}
                    />
                    <p style={{ fontSize: 12, color: C.textLight, margin: "4px 0 0", textAlign: "center" }}>
      ...
      <div style={{ minHeight: "100vh", background: C.bg, backgroundSize: themeMode === "rainbow" ? "400% 400%" : undefined, animation: themeMode === "rainbow" ? "rainbowShift 18s ease infinite" : undefined, fontFamily: "'Patrick Hand', cursive", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "14px 28px", position: "relative", zIndex: 20, background: `${C.card}cc`, backdropFilter: "blur(8px)", borderBottom: `1px solid ${C.fur2}20`, gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <p style={{ color: C.textLight, margin: 0, fontSize: 13 }}>
              Welcome, {user?.name}! · {myClassStudents.length} student{myClassStudents.length !== 1 ? "s" : ""} in your class
            </p>
            {/* Class code pill — click to copy to clipboard so the teacher can share it */}
            {user?.classCode && (
              <div
                onClick={() => {
                  try {
                    navigator.clipboard?.writeText(user.classCode);
                    notify(`Class code ${user.classCode} copied — share with your students!`);
                  } catch { /* clipboard API not available */ }
                }}
                title="Click to copy. Share this with students so they can join your class on signup."
                style={{
                  display: "inline-flex", alignItems: "center", gap: 6,
                  padding: "3px 10px", borderRadius: 999,
                  background: `${C.gold}25`, border: `1.5px solid ${C.gold}60`,
                  fontSize: 12, color: C.text, fontWeight: 700,
                  cursor: "pointer", alignSelf: "flex-start",
                  letterSpacing: 1, fontFamily: "'Patrick Hand', cursive",
                }}
              >
                🎫 Class code: <span style={{ color: C.gold, fontWeight: 700 }}>{user.classCode}</span>
              </div>
            )}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-end", flexWrap: "wrap" }}>
            ...
            {/* ── Simplified teacher nav ──
                Single segmented switcher for the two main views (Roster / Hot Spring).
                Packs and Yearbook open as full-screen overlays (modals), so they
                live as small icon buttons. Add Student + Logout stay as outliers. */}
            <div role="tablist" aria-label="Teacher view" style={{
              display: "inline-flex", padding: 3, background: `${C.snow1}90`,
              borderRadius: 14, border: `1.5px solid ${C.fur2}25`,
            }}>
              {[
                { key: "roster", label: "👥 Students" },
                { key: "hotspring", label: "♨️ Hot Spring" },
              ].map(t => (
                <button
                  key={t.key}
                  role="tab"
                  aria-selected={teacherView === t.key}
                  onClick={() => { SFX.click(); setTeacherView(t.key); }}
                  style={{
                    padding: "8px 16px", borderRadius: 11, border: "none",
                    background: teacherView === t.key ? C.card : "transparent",
                    color: C.text,
                    fontFamily: "'Patrick Hand', cursive", fontSize: 15, fontWeight: 700,
                    cursor: "pointer",
                    boxShadow: teacherView === t.key ? "0 2px 8px rgba(0,0,0,0.08)" : "none",
                    transition: "all 0.2s",
                  }}>
                  {t.label}
                </button>
            ...
            <button
              ...
              title="Browse and assign question packs"
              style={{ padding: "9px 14px", borderRadius: 12, border: `2px solid ${C.gold}30`, background: showStudyPacks ? C.gold : `${C.card}dd`, color: showStudyPacks ? "white" : C.text, fontFamily: "'Patrick Hand', cursive", fontSize: 15, cursor: "pointer", fontWeight: 600 }}>
              📚 Packs
            </button>
            <button
              onClick={logout}
              title="Log out"
              style={{ padding: "9px 12px", borderRadius: 12, border: `2px solid ${C.fur2}30`, background: `${C.card}dd`, color: C.textLight, fontFamily: "'Patrick Hand', cursive", fontSize: 16, cursor: "pointer", minWidth: 42 }}>
              🚪
            </button>
        ...
        {/* ─── ORPHAN STUDENT ASSIGNMENT MODAL ──
            Students who existed before class scoping was added (or who self-signed
            up without a teacher) have no `teacherId`. When a teacher logs in and
            orphans exist, this modal pops up so they can claim (move to my class)
            or skip (leave for another teacher to claim) each one. */}
        {showOrphanModal && orphanStudents.length > 0 && (
          <div style={modalBackdropStyle} onClick={() => setShowOrphanModal(false)}>
            <div style={{ ...modalCardStyle, width: 560, maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div>
                  <h2 style={{ margin: 0, color: C.text, fontSize: 22 }}>👋 Unassigned Students</h2>
                  <p style={{ margin: "4px 0 0", color: C.textLight, fontSize: 14 }}>
                    {orphanStudents.length} student{orphanStudents.length === 1 ? "" : "s"} {orphanStudents.length === 1 ? "isn't" : "aren't"} assigned to any teacher yet. Claim the ones who belong in your class, or skip them for another teacher to claim.
                  </p>
                </div>
                <button onClick={() => setShowOrphanModal(false)} style={{ background: "none", border: "none", fontSize: 22, color: C.textLight, cursor: "pointer" }}>✕</button>
              </div>

              <div style={{ display: "grid", gap: 8, marginBottom: 16 }}>
                {orphanStudents.map(s => {
                  const choice = orphanAssignments[s.id];
                  return (
                    <div key={s.id} style={{
                      display: "flex", justifyContent: "space-between", alignItems: "center",
                      padding: "12px 16px", borderRadius: 12,
                      background: choice === "claim" ? `${C.green}15` : choice === "skip" ? "rgba(0,0,0,0.04)" : `${C.fur2}10`,
                      border: `1.5px solid ${choice === "claim" ? C.green + "50" : choice === "skip" ? "rgba(0,0,0,0.10)" : C.fur2 + "30"}`,
                      transition: "all 0.2s",
                    }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, color: C.text, fontSize: 16 }}>{s.name}</div>
                        <div style={{ fontSize: 12, color: C.textLight }}>@{s.username} · {s.points || 0} ★</div>
                      </div>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button
                          onClick={() => setOrphanAssignments(prev => ({ ...prev, [s.id]: "claim" }))}
                          style={{
                            padding: "6px 12px", borderRadius: 999,
                            background: choice === "claim" ? C.green : "transparent",
                            color: choice === "claim" ? "white" : C.green,
                            border: `1.5px solid ${C.green}`,
                            fontFamily: "'Patrick Hand', cursive", fontWeight: 700, fontSize: 13,
                            cursor: "pointer",
                          }}
                        >
                          {choice === "claim" ? "✓ Mine" : "Claim"}
                        </button>
                        <button
                          onClick={() => setOrphanAssignments(prev => ({ ...prev, [s.id]: "skip" }))}
                          style={{
                            padding: "6px 12px", borderRadius: 999,
                            background: choice === "skip" ? "#888" : "transparent",
                            color: choice === "skip" ? "white" : "#888",
                            border: "1.5px solid #888",
                            fontFamily: "'Patrick Hand', cursive", fontWeight: 700, fontSize: 13,
                            cursor: "pointer",
                          }}
                        >
                          {choice === "skip" ? "✓ Skipped" : "Skip"}
                        </button>
                      </div>
                    </div>
                  );
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                <button
                  onClick={() => { setShowOrphanModal(false); setOrphanAssignments({}); }}
                  style={{
                    padding: "10px 18px", borderRadius: 12,
                    background: "transparent", border: "1.5px solid rgba(0,0,0,0.15)",
                    color: C.textLight, fontFamily: "'Patrick Hand', cursive", fontSize: 15, cursor: "pointer",
                  }}
                >
                  Decide later
                </button>
                <button
                  onClick={async () => {
                    // Apply claims — for each "claim" choice, write teacherId=user.id to that student.
                    // "Skip" choices are left alone so another teacher can claim them next.
                    const claims = Object.entries(orphanAssignments).filter(([, c]) => c === "claim");
                    if (claims.length === 0) { setShowOrphanModal(false); return; }
                    const newS = students.map(s => {
                      const claim = orphanAssignments[s.id];
                      if (claim === "claim") return { ...s, teacherId: user.id };
                      return s;
                    });
                    setStudents(newS);
                    // Fire-and-forget Firestore writes for each claimed student
                    for (const [sid] of claims) {
                      try { await updateStudent(sid, { teacherId: user.id }); }
                      catch (e) { console.error("Orphan claim failed:", sid, e); }
                    }
                    setShowOrphanModal(false);
                    setOrphanAssignments({});
                    notify(`✓ Added ${claims.length} student${claims.length === 1 ? "" : "s"} to your class`);
                  }}
                  style={{
                    padding: "10px 18px", borderRadius: 12,
                    background: C.green, border: "none", color: "white",
                    fontFamily: "'Patrick Hand', cursive", fontSize: 15, fontWeight: 700, cursor: "pointer",
                  }}
                >
                  Apply
                </button>
                ...
                <div key={s.id} style={{ padding: "12px 14px", borderRadius: 14, background: `${C.snow1}80`, marginBottom: 10 }}>
                  ...
                  <div>
                    ...
                    {studentMissions.map(m => {
                      const completionData = (s.completions || {})[`mission:${m.id}`];
                      const isCompleted = !!completionData?.completed;
                      const bestScore = completionData?.bestScore;
                      const scoreTotal = completionData?.scoreTotal;
                      return (
                        <div key={m.id} style={{
                          display: "flex", alignItems: "center", gap: 4, padding: "4px 8px",
                          background: isCompleted ? `${C.green}25` : `${C.green}15`,
                          borderRadius: 6, marginBottom: 2,
                          border: isCompleted ? `1px solid ${C.green}50` : "1px solid transparent",
                        }}>
                          <div style={{ flex: 1, fontSize: 12, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            ...
                            {isCompleted && bestScore !== undefined && bestScore !== null && (
                              <span style={{ marginLeft: 6, color: C.green, fontWeight: 700 }}>
                                ✓ {bestScore}{scoreTotal ? `/${scoreTotal}` : ""}
                              </span>
                            )}
                            {!isCompleted && (
                              <span style={{ marginLeft: 6, color: C.textLight, fontStyle: "italic" }}>
                                · not done
                              </span>
                            )}
                          </div>
                          <button onClick={() => setMissionResultsTarget({ studentId: s.id, missionId: m.id })}
                            title="View results"
                            style={{ padding: "2px 8px", borderRadius: 6, border: "none", background: `${C.water1}30`, color: C.text, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
                            📊
                          </button>
        ...
        {/* MISSION RESULTS MODAL — teacher-only view of a student's attempts on a mission */}
        {missionResultsTarget && (() => {
          const { studentId, missionId } = missionResultsTarget;
          const targetStudent = students.find(st => st.id === studentId);
          const studentMissions = missions[studentId] || [];
          const targetMission = studentMissions.find(m => m.id === missionId);
          if (!targetStudent || !targetMission) {
            return null;
          }
          const completionData = (targetStudent.completions || {})[`mission:${missionId}`];
          const isCompleted = !!completionData?.completed;
          const latestAttempt = completionData?.latestAttempt;
          const wrongs = latestAttempt?.wrongAnswers || [];
          const score = latestAttempt?.score;
          const scoreTotal = latestAttempt?.scoreTotal;
          const totalQs = targetMission.questions?.length || 0;
          ...
          const gameTypeLabel = ({
            ...
            crush: "🌸 Monkey Crush",
            quiz: "📝 Quiz Mode",
          })[latestAttempt?.gameType] || "Unknown mode";
          const attemptCount = completionData?.attempts || 0;
          return (
            <div style={modalBackdropStyle} onClick={() => setMissionResultsTarget(null)}>
              <div style={{
                ...modalCardStyle, width: 640, maxWidth: "95vw",
                maxHeight: "90vh", overflowY: "auto",
              }} onClick={e => e.stopPropagation()}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div>
                    <h2 style={{ margin: 0, color: C.text, fontSize: 22 }}>📊 {targetMission.name}</h2>
                    <p style={{ margin: 0, color: C.textLight, fontSize: 13 }}>
                      {targetStudent.name} · {totalQs} questions · {targetMission.points}★ reward
                    </p>
                  </div>
                  <button onClick={() => setMissionResultsTarget(null)}
                    style={{ background: "none", border: "none", fontSize: 22, color: C.textLight, cursor: "pointer" }}>
                    ✕
                  </button>
                </div>
                ...
                {!isCompleted ? (
                  <div style={{
                    background: `${C.fur2}20`, padding: "20px 18px", borderRadius: 14,
                    textAlign: "center", marginTop: 12,
                  }}>
                    <div style={{ fontSize: 36, marginBottom: 8 }}>⏳</div>
                    <div style={{ color: C.text, fontWeight: 700, fontSize: 16 }}>Not started yet</div>
                    <div style={{ color: C.textLight, fontSize: 13, marginTop: 4 }}>
                      {targetStudent.name} hasn't completed this mission.
                      {attemptCount > 0 && ` (${attemptCount} attempt${attemptCount === 1 ? "" : "s"} in progress)`}
                    </div>
                  </div>
                ) : (
                  <>
                    {/* Best-score banner */}
                    <div style={{
                      background: `linear-gradient(135deg, ${C.green}25, ${C.gold}25)`,
                      padding: "14px 18px", borderRadius: 14, marginTop: 12,
                      border: `2px solid ${C.green}40`,
                    }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                        <div>
                          <div style={{ color: C.green, fontWeight: 700, fontSize: 16 }}>✓ Completed</div>
                          <div style={{ color: C.textLight, fontSize: 12 }}>
                            Mode: {gameTypeLabel} · {attemptCount} attempt{attemptCount === 1 ? "" : "s"}
                            {completedAt && (
                              <> · {new Date(completedAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</>
                            )}
                          </div>
                        </div>
                        {score !== undefined && score !== null && scoreTotal && (
                          <div style={{
                            background: C.card, padding: "6px 14px", borderRadius: 999,
                            fontSize: 18, fontWeight: 700, color: C.text,
                          }}>
                            {score}/{scoreTotal} <span style={{ color: C.textLight, fontSize: 12 }}>correct</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: 16 }}>
                      {wrongs.length === 0 ? (
                        <div style={{
                          background: `${C.green}15`, padding: "16px 14px",
                          borderRadius: 12, textAlign: "center",
                        ...
                        <>
                          <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 8 }}>
                            ❌ Wrong answers in latest attempt ({wrongs.length})
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            {wrongs.map((w, i) => (
                              <div key={i} style={{
                                background: `${C.accent}10`, padding: "10px 12px",
                                borderRadius: 10, border: `1px solid ${C.accent}30`,
                              }}>
                                <div style={{ fontSize: 13, color: C.text, marginBottom: 4, fontWeight: 600 }}>
                                  Q{(w.questionIdx ?? i) + 1}: {w.question}
                                </div>
                                <div style={{ fontSize: 12, marginBottom: 2 }}>
                                  <span style={{ color: C.textLight }}>Their answer: </span>
                                  <span style={{ color: C.accentDark, fontWeight: 700 }}>
                                    {"ABCD"[w.chosen] || "?"} {w.chosenText ? `— ${w.chosenText}` : ""}
                                  </span>
                                </div>
                                <div style={{ fontSize: 12 }}>
                                  <span style={{ color: C.textLight }}>Correct: </span>
                                  <span style={{ color: C.green, fontWeight: 700 }}>
                                    {"ABCD"[w.correct] || "?"} {w.correctText ? `— ${w.correctText}` : ""}
                                  </span>
                                </div>
                              </div>
                            ))}
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 18 }}>
                  <button onClick={() => setMissionResultsTarget(null)} style={primaryBtnStyle}>
                    Close
                  </button>
                </div>
              </div>
  ...
  if (screen === "student") {
    const me = students.find(s => s.id === user?.id);
    // Global rank (against everyone)
    const sorted = [...students].sort((a, b) => b.points - a.points);
    const rank = sorted.findIndex(s => s.id === user?.id) + 1;
    // Class rank (against classmates with the same teacherId)
    // If the student has no teacherId yet (orphan), class rank == global rank.
    const myClassmates = me?.teacherId
      ? students.filter(s => s.teacherId === me.teacherId)
      : students;
    const classSorted = [...myClassmates].sort((a, b) => b.points - a.points);
...
{/* Mission modal - dispatches to the right game type */}
        {showMission && me && (() => {
          ...
          // Stable callback hoisted at component-level so the heavy game
          // components don't see a new onClose prop on every parent re-render.
          const closeFn = closeMission;
          ...
          // Before launching the game, if the student has never seen this game
          // type's tutorial AND hasn't already chosen "skip" this session, show
          // the TutorialPrompt. Saying "Yes" turns on the overlay (handled outside
          // this IIFE so it sits visually above the game). Saying "Skip" persists
          // the seen flag so they aren't asked again.
          const tutorialSeenKey = `tutorial:${gameType}`;
          const hasSeenTutorial = !!me.completions?.[tutorialSeenKey]?.completed;
          const promptDecidedKey = `${activeMission.id}:${gameType}`;
          const tutorialPromptOpen = !hasSeenTutorial && !tutorialPromptDecided[promptDecidedKey];
          if (tutorialPromptOpen) {
            return (
              <TutorialPrompt
                gameType={gameType}
                onYes={() => {
                  setTutorialPromptDecided(prev => ({ ...prev, [promptDecidedKey]: true }));
                  setActiveTutorialGameType(gameType);
                }}
                onSkip={() => {
                  setTutorialPromptDecided(prev => ({ ...prev, [promptDecidedKey]: true }));
                  persistTutorialSeen(gameType);
                }}
              />
            );
          }
        ...
        {/* ── Tutorial bubble overlay ──
            Sits above the currently-running game when the student opted in.
            Independent of the dispatcher above so it survives mid-game re-renders. */}
        {showMission && activeTutorialGameType && (
          <TutorialOverlay
            gameType={activeTutorialGameType}
            onDone={() => {
              persistTutorialSeen(activeTutorialGameType);
              setActiveTutorialGameType(null);
            }}
          />
        )}
        ...
        <div style={{ position: "relative", margin: "8px auto 0", width: "96%", maxWidth: 1300, height: "calc(100vh - 90px)", borderRadius: 20, overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.1)" }}>
          ...
          {!leaderboardOpen ? (
            ...
            <div style={{
              ...
              {/* Class vs Global tab switcher */}
              <div style={{ display: "flex", gap: 4, marginBottom: 10, background: "rgba(0,0,0,0.04)", borderRadius: 999, padding: 3 }}>
                <button
                  onClick={() => setLeaderboardTab("class")}
                  style={{
                    flex: 1, padding: "4px 8px", borderRadius: 999, border: "none",
                    background: leaderboardTab === "class" ? C.gold : "transparent",
                    color: leaderboardTab === "class" ? "white" : C.textLight,
                    fontFamily: "'Patrick Hand', cursive", fontSize: 11, fontWeight: 700,
                    cursor: "pointer",
                  }}
                >
                  🏫 Class
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: "4px 8px", borderRadius: 999, border: "none",
                    background: leaderboardTab === "global" ? C.gold : "transparent",
                    color: leaderboardTab === "global" ? "white" : C.textLight,
                    fontFamily: "'Patrick Hand', cursive", fontSize: 11, fontWeight: 700,
                    cursor: "pointer",
                  }}
                >
              ...
              {/* Top 3 - always visible. Data source depends on the tab. */}
              {(() => {
                const lbList = leaderboardTab === "class" ? classSorted : sorted;
                return (
              <>
              <div style={{ marginBottom: lbList.length > 3 ? 8 : 0 }}>
                {lbList.slice(0, 3).map((s, i) => {
                  const isMe = s.id === me?.id;
                  const podiumBg = i === 0 ? `${C.gold}25` : i === 1 ? "#b0b0b020" : "#cd7f3220";
                  return (
                    <div key={s.id} style={{
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                ...
                {classSize !== students.length && (
                  <span style={{ fontSize: 13, color: C.textLight, fontWeight: 600 }} title={`Global rank #${rank} of ${students.length}`}>
                    🌍 #{rank}
                  </span>
                )}
              </div>
